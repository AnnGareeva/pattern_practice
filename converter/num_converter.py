...
class Converter:
    _roman_symbols = {'m': 1000, 'd': 500, 'c': 100, 'l': 50, 'x': 10, 'v': 5, 'i': 1}
    ...
    def _valid_arab_value(self, arab_num):
        ...
        if arab_num > 0:
            if arab_num > 3000:
                raise OverflowError('Input value must be < 3000')
        else:
            raise ValueError('Input value must be > 0')

    def arab_to_roman(self, arab_num):
        dict_roman = Converter._roman_symbols
        self._valid_arab_value(arab_num)
