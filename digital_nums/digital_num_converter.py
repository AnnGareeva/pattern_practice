...
def convert_data_to_num_list(digital_num_obj, data):
    ...
    while start_vertical < len(data) - num_size_vertical + 1:
        ...
        if counter < stop_counter: break
    return num_list

def check_valid_size(line):
    if len(line) % 4 != 0 and len(line) % 4 != 3:
...
def read_from_file(filename):
    if os.path.exists(filename):
        ...
        dn = DigitalNumber()
        return convert_data_to_num_list(dn, data)
    else:
...
if __name__ == "__main__":
    ...
    if args.input:
        result_list = read_from_file(args.input)
    else:
        result_list = read_from_terminal()
